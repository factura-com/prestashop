...
class AdminFacturaController extends ModuleAdminController
{
    ...
    public function initContent()
    {
        ...
        $response = json_decode(Curls::adminCurl($url, $keyapi, $keysecret));
        ...
        $url_ajax = $this->context->link->getAdminLink('AdminFactura');
        //var_dump($url_ajax);
    ...
    public function ajaxProcessDownloadFile()
    {
        ...
        $response = Curls::adminCurl($url, $keyapi, $keysecret, true);

        // Verificar si la respuesta es válida
        if (!$response) {
            header('Content-type: application/json');
            echo json_encode(array('error' => 'No se pudo obtener el archivo'));
            return;
        }
        ...
        $filename = 'factura_' . $uid . '.' . $extension;
